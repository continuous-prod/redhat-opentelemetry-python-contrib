...
#
"""Autometed instrumentaition for Python scripts

This create an OpenTelementry span that wraps an entire script and includes
information about the command line arguments its got and and exceptions raised
"""
import atexit
import os
import sys
...
class UndoingEnvironGetterSetter(Setter[os._Environ], Getter[os._Environ]):
    ...
    def undo(self) -> None:
        ...
        for carrier, ub in self._undo_buffer.values():
            for key, vlist in ub.items():
                if vlist is None:
                    del carrier[key]
                else:
                    carrier[key] = vlist[0]
...
class ScriptInstrumentor(BaseInstrumentor):
    ...
    def _start_span(
        ...
        # properly
        self._span_ctx.__enter__()  # pylint: disable=no-member
    ...
    def _instrumented_exit(self, code: object = 0, /) -> NoReturn:
        """Figure out what the process exist code is going to be by immitating
        sys.exit's behvaiour where it eithr uses the int passed to it, treats
